...
classes_path = os.path.join(prefix, "jdi-qasp-ml", "data/old_dataset/classes.txt")
dataset_path = os.path.join(prefix, "jdi-qasp-ml", "data/old_dataset")

EPSILON = 1e-6

PRIORITY_SCALAR = defaultdict(
    int,
    {
        "A": EPSILON,
        "BUTTON": EPSILON,
...
def assign_labels(
    ...
    img_width: int = 0,
    img_height: int = 0,
    classes_file_path: str = classes_path,
    verbose=False,
) -> pd.DataFrame:
    """
    ...
    os.makedirs(f"{dataset_path}/cache-labels", exist_ok=True)

    ds_name = re.split(r"[/\\]+", re.sub(r"\.txt$", "", annotations_file_path))[-1]
    logger.info(
        f" Dataset: {ds_name}, assign labels from annotation file: {annotations_file_path}"
    )
    ...
    cache_file_name = re.sub(
        r"\.txt$",
        ".pkl",
        re.sub(
            r"^dataset[/\\]{1}annotations[/\\]{1}",
            f"{dataset_path}/cache-labels/",
    ...
    if os.path.exists(image_file_name):
        logger.info(
            f"Image's (width,height) are taken from image file: {image_file_name}"
        )
    ...
    for bb in tqdm(_ann, desc=f"Assign labels for dataset [{ds_name}]"):
        ...
        for idx, r in _boxes_df.iterrows():
            ...
            iou = (
                iou_xywh(
                    from_yolo(x, y, w, h, img_width, img_height),
                    (r.x, r.y, r.width, r.height),
                )
                + PRIORITY_SCALAR[r.tag_name]
            )

            if (
                iou >= best_iou
            ):  # We have to use >=, because the next tag might be more important
                (
                    best_idx,
                    best_iou,
                    best_tag,

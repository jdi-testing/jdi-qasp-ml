...
def check_is_inside(parent_rect, child_rect):
    """
        rect: (x,y,w,h),
        returns (fully_inside, patrually_inside, intersectio_over_union, center_is_inside)
    """
    ...
    # calculate coordinates of child's center
    cx = cx1 + cw / 2.0
    cy = cy1 + ch / 2.0
    ...
    if (px1 <= cx) and (px2 >= cx) and (py1 <= cy) and (py2 >= cy):
        center_is_inside = 1.0
    else:
        center_is_inside = 0.0
    ...
    if cx1_inside and cx2_inside and cy1_inside and cy2_inside:
        return (1.0, 0.0, iou, center_is_inside)
    else:
        return (0.0, 1.0, iou, center_is_inside)
